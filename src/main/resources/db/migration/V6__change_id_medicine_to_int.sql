ALTER TABLE "Dosage" DROP CONSTRAINT IF EXISTS id_medicine;
ALTER TABLE "Incompatibility" DROP CONSTRAINT IF EXISTS id_medicine;
ALTER TABLE "Incompatibility" DROP CONSTRAINT IF EXISTS id_medicine_inc;
...
UPDATE "Incompatibility" SET new_id = CAST(SUBSTRING(id::text FROM 1 FOR 8) AS INT);
UPDATE "Incompatibility" SET new_id_inc = CAST(SUBSTRING(id::text FROM 1 FOR 8) AS INT);
UPDATE "Dosage" SET new_id = CAST(SUBSTRING(id::text FROM 1 FOR 8) AS INT);
